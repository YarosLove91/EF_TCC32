/*
	Copyright 2023 Efabless Corp.

	Author: Mohamed Shalan (mshalan@aucegypt.edu)

	This file is auto-generated by wrapper_gen.py on 2023-10-19

	Licensed under the Apache License, Version 2.0 (the "License");
	you may not use this file except in compliance with the License.
	You may obtain a copy of the License at

	    http://www.apache.org/licenses/LICENSE-2.0

	Unless required by applicable law or agreed to in writing, software
	distributed under the License is distributed on an "AS IS" BASIS,
	WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
	See the License for the specific language governing permissions and
	limitations under the License.

*/

`timescale			1ns/1ns
`default_nettype	none

module EF_TCC32_apb (
	input	wire 		ext_clk,
	input	wire 		PRESETn,

	output	wire 		irq,

    output wire gpio_pwm,	// PWM gpio output

	// bus slave connections - periph bus and eu_direct_link
	APB.Slave           apb_slave
);
	localparam[15:0] TIMER_REG_ADDR 		= 	16'h0000;
	localparam[15:0] PERIOD_REG_ADDR 		= 	16'h0004;
	localparam[15:0] COUNTER_REG_ADDR 		= 	16'h0008;
	localparam[15:0] COUNTER_MATCH_REG_ADDR = 	16'h000c;
	localparam[15:0] CONTROL_REG_ADDR 		= 	16'h0010;
	localparam[15:0] PWM_COMP_VAL_ADDR		= 	16'h0014;	// PWM compare register
	localparam[15:0] ICR_REG_ADDR 			= 	16'h0f00;
	localparam[15:0] RIS_REG_ADDR 			= 	16'h0f04;
	localparam[15:0] IM_REG_ADDR 			= 	16'h0f08;
	localparam[15:0] MIS_REG_ADDR 			= 	16'h0f0c;

	reg	[31:0]	PERIOD_REG;
	reg	[31:0]	COUNTER_MATCH_REG;
	reg	[31:0]	CONTROL_REG;
	reg	[2:0]	RIS_REG;
	reg	[2:0]	ICR_REG;
	reg	[2:0]	IM_REG;

	wire[31:0]	tmr;
	wire[31:0]	TIMER_REG	= tmr;
	wire[31:0]	period		= PERIOD_REG[31:0];
	wire[31:0]	cp_count;
	wire[31:0]	COUNTER_REG	= cp_count;
	wire[31:0]	ctr_match	= COUNTER_MATCH_REG[31:0];

	reg [31:0]	PWM_COMP_VAL_REG;

	wire		en			= CONTROL_REG[0:0];
	wire		tmr_en		= CONTROL_REG[1:1];
	wire 		pwm_en  	= CONTROL_REG[2:2];		// PWM_enable
	wire		cp_en		= CONTROL_REG[3:3];
	wire[3:0]	clk_src		= CONTROL_REG[11:8];
	wire		up			= CONTROL_REG[16:16];
	wire		one_shot	= CONTROL_REG[17:17];
	wire[1:0]	cp_event	= CONTROL_REG[25:24];
	wire		to_flag;
	wire		_TO_FLAG_	= to_flag;
	wire		cp_flag;
	wire		_CP_FLAG_	= cp_flag;
	wire		match_flag;
	wire		_MATCH_FLAG_= match_flag;
	wire[2:0]	MIS_REG		= RIS_REG & IM_REG;
	wire		ctr_in		= ext_clk;
	wire		apb_valid	= apb_slave.psel & apb_slave.penable;
	wire		apb_we		= apb_slave.pwrite & apb_valid;		// PSEL & PENABLE & PWRITE
	wire		apb_re		= ~apb_slave.pwrite & apb_valid;
	wire		_clk_		= apb_slave.pclk;
	wire		_rst_		= ~PRESETn;

	EF_TCC32 inst_to_wrap (
		.clk(_clk_),
		.rst_n(~_rst_),
		.ctr_in(ctr_in),
		.pwm_out_pin(gpio_pwm),			// PWM signal out for GPIO
		.pwm_cmp_in(PWM_COMP_VAL_REG),	// PWM input compare value
		.period(period),
		.ctr_match(ctr_match),
		.tmr(tmr),
		.cp_count(cp_count),
		.clk_src(clk_src),
		.to_flag(to_flag),
		.match_flag(match_flag),
		.tmr_en(tmr_en),
		.pwm_en(pwm_en),				// PWM_enable 
		.one_shot(one_shot),
		.up(up),
		.cp_en(cp_en),
		.cp_event(cp_event),
		.cp_flag(cp_flag),
		.en(en)
	);

	// Декодеры адреса

	// Timer Period Register 
	// [offset: 0x04, RW]
	always @(posedge apb_slave.pclk or negedge PRESETn) 
		if(~PRESETn) PERIOD_REG <= 0; 
		else if(apb_we & (apb_slave.paddr[15:0]==PERIOD_REG_ADDR))
			PERIOD_REG <= apb_slave.pwdata[32-1:0];
	
	// Counter Match Register 
	// [offset: 0x0C, RW]
	always @(posedge apb_slave.pclk or negedge PRESETn) 
		if(~PRESETn) COUNTER_MATCH_REG <= 0; 
		else if(apb_we & (apb_slave.paddr[15:0]==COUNTER_MATCH_REG_ADDR)) 
			COUNTER_MATCH_REG <= apb_slave.pwdata[32-1:0];

	// Control Register 
	// [offset: 0x10, RW]
	always @(posedge apb_slave.pclk or negedge PRESETn) 
		if(~PRESETn) CONTROL_REG <= 0; 
		else if(apb_we & (apb_slave.paddr[15:0]==CONTROL_REG_ADDR)) 
			CONTROL_REG <= apb_slave.pwdata[32-1:0];

	// PWM compare value register
	// [offset: 0x14, RW]
		always @(posedge apb_slave.pclk or negedge PRESETn) 
			if(~PRESETn) PWM_COMP_VAL_REG <= 0; 
			else if(apb_we & (apb_slave.paddr[15:0]==PWM_COMP_VAL_ADDR)) 
				PWM_COMP_VAL_REG <= apb_slave.pwdata[32-1:0];

	// Interrupt Mask Register
	// [offset: 0xF08, RW]
	always @(posedge apb_slave.pclk or negedge PRESETn) 
		if(~PRESETn) IM_REG <= 0; 
		else if(apb_we & (apb_slave.paddr[15:0]==IM_REG_ADDR)) 
			IM_REG <= apb_slave.pwdata[3-1:0];

	// Raw Interrupts Status Register
	// [offset: 0xF00, RO]
	always @(posedge apb_slave.pclk or negedge PRESETn) 
		if(~PRESETn) ICR_REG <= 3'b0; 
		else if(apb_we & (apb_slave.paddr[15:0]==ICR_REG_ADDR)) 
			ICR_REG <= apb_slave.pwdata[3-1:0]; 
				else ICR_REG <= 3'd0;

	// Masked Interrupts Status Register  
	// [offset: 0xF04, RO]
	always @(posedge apb_slave.pclk or negedge PRESETn)
		if(~PRESETn) RIS_REG <= 3'd0;
		else begin
			if(_TO_FLAG_) RIS_REG[0] <= 1'b1; 
				else if(ICR_REG[0]) RIS_REG[0] <= 1'b0;
			if(_CP_FLAG_) RIS_REG[1] <= 1'b1; 
				else if(ICR_REG[1]) RIS_REG[1] <= 1'b0;
			if(_MATCH_FLAG_) RIS_REG[2] <= 1'b1; 
				else if(ICR_REG[2]) RIS_REG[2] <= 1'b0;
		end

	assign irq = |MIS_REG;

	// PRDATA - Шина данных для чтения. Может иметь разрядность до 32 бит.
	// PADDR - Шина адреса. Может иметь разрядность до 32 бит. 
	// Поступает от ведущего устройства, ведомое читает
	assign	apb_slave.prdata = 
			(apb_slave.paddr[15:0] == PERIOD_REG_ADDR) ? PERIOD_REG :
			(apb_slave.paddr[15:0] == COUNTER_MATCH_REG_ADDR) ? COUNTER_MATCH_REG :
			(apb_slave.paddr[15:0] == CONTROL_REG_ADDR) ? CONTROL_REG :
			(apb_slave.paddr[15:0] == RIS_REG_ADDR) ? RIS_REG :
			(apb_slave.paddr[15:0] == ICR_REG_ADDR) ? ICR_REG :
			(apb_slave.paddr[15:0] == IM_REG_ADDR) ? IM_REG :
			(apb_slave.paddr[15:0] == TIMER_REG_ADDR) ? TIMER_REG :
			(apb_slave.paddr[15:0] == COUNTER_REG_ADDR) ? COUNTER_REG :
			(apb_slave.paddr[15:0] == MIS_REG_ADDR) ? MIS_REG :
            (apb_slave.paddr[15:0] == PWM_COMP_VAL_ADDR) ? PWM_COMP_VAL_REG :
			32'hDEADBEEF;
	assign apb_slave.pready = 1'b1;

endmodule
